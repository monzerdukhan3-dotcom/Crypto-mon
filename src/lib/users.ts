...
export async function findUserByEmail(email: string): Promise<AppUser | null> {
  const db = sql();
  const rows = await db`
    select id, email, password_hash as "passwordHash" from users where email = ${email.toLowerCase()} limit 1
  `;
...
export async function createUser(email: string, password: string): Promise<AppUserSummary> {
  const db = sql();
...
export async function listUsers(): Promise<AppUserSummary[]> {
  const db = sql();
  const rows = await db`select id, email, created_at as "createdAt" from users order by created_at asc`;
